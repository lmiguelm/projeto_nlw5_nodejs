  ...
  socket.emit('admin_list_messages_by_user', params, (messages) => {
    ...
    messages.forEach((message) => {
      ...
      if (message.adminId === null) {
        ...
        createDiv.innerHTML += `<span class="admin_date">${dayjs(message.createdAt).format(
          'DD/MM/YYYY HH:mi:ss'
        )}</span>`;
      } else {
...
function sendMessage(id) {
  ...
  createDiv.innerHTML += `<span class="admin_date">${dayjs().format('DD/MM/YYYY HH:mi:ss')}</span>`;
...
socket.on('admin_receive_message', (data) => {
  const connection = connectionsUsers.find((connection) => (connection.socketId = data.socketId));
  ...
  createDiv.innerHTML += `<span class="admin_date">${dayjs(data.message.createdAt).format(
    'DD/MMYYY HH:mm:ss'
  )}</span>`;
